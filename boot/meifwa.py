...
class MeifwaBot(commands.AutoShardedBot):
    def __init__(self, *args, **kwargs):
        for logger in [
            ...
            logging.getLogger(logger).setLevel(
                logging.DEBUG if logger == "meifwa" else logging.INFO
            )
            logging.getLogger(logger).addHandler(LoggingHandler())
        self.logger = logging.getLogger("meifwa")
        super().__init__(
            help_command=None,
    ...
    async def on_connect(self):
        ...
        try:
            ...
            pass
        self.logger.info("Connected to the database: `meifwa.db`")
    ...
    async def on_message(self, message: discord.Message):
        ctx: MeifwaContext = await self.get_context(message, cls=MeifwaContext)
